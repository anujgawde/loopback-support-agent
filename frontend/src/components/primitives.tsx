...
import { SEVERITY, DECISION, STATUSES, CATEGORIES, KB_STATUS, DEFAULT_CATEGORY_STYLE } from '@/lib/tokens';
import type { TicketSeverity, AgentAction, TicketStatus } from '@/lib/types';
import * as I from './icons';

export function CategoryBadge({ name, size = 'md' }: { name: string; size?: 'sm' | 'md' }) {
  const c = CATEGORIES[name] || DEFAULT_CATEGORY_STYLE;
  const cls = size === 'sm' ? 'text-[10px] px-1.5 py-[1px]' : 'text-[10.5px] px-2 py-[2px]';
  return (
    <span
...
export function SeverityPill({ level, withLabel = true }: { level: TicketSeverity; withLabel?: boolean }) {
...
export function ScoreBar({ value, color = '#EE5734', height = 4 }: { value: number; color?: string; height?: number }) {
  return (
    <div className="w-full rounded-full bar-fill overflow-hidden" style={{ height, background: '#F1F1EF' }}>
...
type BtnVariant = 'primary' | 'secondary' | 'ghost' | 'quiet' | 'outline' | 'danger';
...
export function Btn({
  ...
  const variants: Record<BtnVariant, string> = {
    primary: 'bg-accent text-white hover:bg-accent2 font-medium border border-accent2',
    secondary: 'bg-ink text-white hover:bg-ink2 font-medium',
    ghost: 'bg-card text-ink2 hover:text-ink hover:bg-sunken border border-line',
    quiet: 'bg-transparent text-muted hover:text-ink',
    outline: 'bg-card text-accent hover:bg-accent-bg border border-accent-line font-medium',
...
export function CopyButton({ text, label = 'Copy to Clipboard', size = 'sm', variant = 'ghost' }: {
  text: string;
  label?: string;
  size?: 'sm' | 'md';
  variant?: BtnVariant;
  ...
  return (
    <Btn size={size} variant={variant} onClick={onClick} icon={done
...
export function ToolPill({ children, kind = 'default' }: { children: ReactNode; kind?: 'default' | 'success' | 'warn' | 'danger' | 'accent' }) {
...
export function ConfidencePill({ value }: { value: number | null }) {
  ...
  const color = value >= 0.85 ? '#16A34A' : value >= 0.7 ? '#CA8A04' : value >= 0.55 ? '#A16207' : '#A8A29E';
  return (
    <div className="flex items-center gap-2 min-w-[88px]">
      <span className="font-mono text-[12px] tabular-nums w-9 text-right" style={{ color }}>{pct}%</span>
      <div className="flex-1 h-1 rounded-full overflow-hidden" style={{ background: '#F1F1EF' }}>
...
export function FilterChip({ icon, label, value, onRemove }: {
  icon?: ReactNode;
  label: string;
  value: string;
  onRemove?: () => void;
  ...
  return (
    <span className="inline-flex items-center gap-1.5 h-7 px-2 rounded-md border border-line bg-card text-[12px] text-ink2 shadow-card">
      ...
      {onRemove && (
        <button onClick={onRemove} className="text-muted2 hover:text-ink ml-0.5">
      ...
      {tabs.map(t => (
        <button
          ...
          className={`relative pb-3 text-[13px] transition-colors ${
            value === t.id ? 'text-ink font-medium' : 'text-muted hover:text-ink'
          }`}
        >
          <span className="inline-flex items-center gap-1.5">
            {t.label}
            {t.count != null && (
              <span className={`text-[11px] font-mono px-1.5 rounded ${value === t.id ? 'bg-accent-bg text-accent' : 'bg-sunken text-muted'}`}>
                {t.count}
              </span>
            )}
          </span>
          {value === t.id && <span className="absolute -bottom-px left-0 right-0 h-[2px] bg-accent rounded-full" />}
        </button>
      ))}
    </div>
  );
...
export function Field({ label, children }: { label: string; children: ReactNode }) {
  return (
    <div>
      <div className="text-[10.5px] tracking-wide font-medium text-muted2 uppercase mb-1.5">{label}</div>
