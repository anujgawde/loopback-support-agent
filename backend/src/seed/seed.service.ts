  ...
  return [
    {
      ...
      symptoms: `Customer connected their third-party account but the ${company} dashboard shows no data, no activity. Often reported within 24-48 hours of initial setup.`,
      rootCause: `Most common: OAuth app not installed on the correct account, or installed with limited scope/access. Second most common: webhooks failing silently due to network-level IP allowlists blocking ${company} endpoints.`,
      resolution: `1. Verify the OAuth app is installed on the correct account with full required scopes. 2. Confirm access permissions cover the correct resources. 3. Check any IP allowlist - ${company} endpoints must be whitelisted. 4. Verify webhook deliveries in the third-party settings. 5. If all looks correct, wait 24-48h for initial data ingestion. 6. Escalate to engineering if webhooks show 4xx/5xx responses.`,
      triggerPhrases: [
    ...
    },
    {
      ...
      category: "Metrics",
      symptoms: `Customer reports ${company} metrics don't match their expectations. Common complaints: "user X did a lot but has low score", "metrics dropped suddenly", "new members show higher metrics than veterans".`,
      rootCause:
    ...
    },
    {
      ...
      symptoms: `Customer has team members using specific tools but the ${company} insights dashboard shows no attribution data or reports 0% usage.`,
      rootCause:
    ...
    },
    {
      ...
      rootCause: `Enterprise SSO requires SCIM provisioning to be fully synced before login works. New users added to the IdP (Okta, Azure AD, etc.) may not yet be provisioned in ${company}. Also: email domain mismatch between IdP and org settings.`,
    ...
    },
    {
      ...
      category: "Integration",
      symptoms: `Some users who are active don't appear on the ${company} dashboard. Their activity exists in the connected platform but the system doesn't show them.`,
      rootCause: `${company} matches users by email. If a user has a different email in their tool config than what's in their ${company} profile, they won't be matched. Also: users on excluded resources or using alias accounts may not appear.`,
...
export class SeedService {
  ...
  async seed(): Promise<void> {
    const company =
      this.configService.get<string>("COMPANY_NAME") || "YourCompany";
    const articles = buildSeedArticles(company);

    this.logger.log(
      `Seeding ${articles.length} KB articles for "${company}"...`,
    );
